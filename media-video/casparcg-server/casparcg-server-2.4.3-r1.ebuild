...
src_install() {
	...
	# ```
	#
	# The project-provided script `copy_deps.sh` uses `ldd` to collects _all_
	...
	dolib.so accelerator/libaccelerator.so
	dolib.so common/libcommon.so
	dolib.so core/libcore.so
	dolib.so modules/artnet/libartnet.so
	dolib.so modules/decklink/libdecklink.so
	dolib.so modules/ffmpeg/libffmpeg.so
	dolib.so modules/image/libimage.so
	dolib.so modules/newtek/libnewtek.so
	dolib.so modules/oal/liboal.so
	dolib.so modules/screen/libscreen.so
	dolib.so protocol/libprotocol.so
